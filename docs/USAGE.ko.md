...
- `--partition-keysound-tracks 2,5,6,10`: **원곡 파형 보존 키음 슬라이스.** 해당 트랙의 스템을 노트 온셋 기준 **빈틈·겹침 없는 타일**로 자릅니다 — 각 키음 = [이 온셋, 다음 온셋), 자르는 위치는 채보가 실제로 트리거하는 양자화 격자 시각. 순서대로 재생하면 **스템 파형이 샘플 단위로 재조립**되고(겹침 더블링·페이드 손실 없음), 각 타격은 다음 타격까지의 여운을 자연스럽게 통째로 가집니다. 해당 트랙은 키음 재사용·clip-overlap을 우회(동일 내용 dedupe는 적용 → 반복 패턴은 자동 압축). 타격감 악기(킥·햇·스네어·베이스)에 적합하고, `--master-residual-bed`와 함께 쓰면 잔차가 순수 마스터 글루만 담게 되어 가장 깨끗합니다.
